...
const LoginPage = () => {
  ...
  const onSubmitHandler = (event) => {
    ...
    const credentials = {
     ...
     const apiState = currentState === "Sign up" ? "signup" :"login";
    dispatch(logIn(apiState,credentials));
  }
  ...
  return (
    ...
    justify-center gap-8 sm:justify-evenly max-sm:flex-col backdrop-blur-2xl '>
      ...
      <button className='py-3 bg-linear-to-r from-purple-400 to-violet-600 text-white rounded-md cursor-pointer ' type='submit'>{currentState === 'Sign up' ? 'Create Account' : 'Login Now'}</button>
      ...
      <div className='flex flex-col gap-2 '>
        {currentState === 'Sign up' ? (<p className='text-sm text-gray-600'>Already have an account? <span onClick={() => {setCurrentState("Login") ; setIsDataSubmitted(false)}} className='font-medium
         text-violet-500 cursor-pointer'>login here</span></p>) : (<p className='text-sm text-gray-600'>Create an account <span onClick={() => {setCurrentState("Sign up") ; setIsDataSubmitted(false)}} className='font-medium text-violet-500 cursor-pointer '>Click here </span></p>) }
      </div>
